...
def _extract_all_tps(text: str) -> List[float]:
    ...
    # Pattern 6b: TP1  4550 (sans séparateur, deux espaces)
    if not tps:
        for m in re.finditer(r"\bTP\s*(\d+)\s{1,4}(\d{4,}(?:\.\d+)?)", text, re.IGNORECASE):
            n_tp = int(m.group(1))
            val  = float(m.group(2))
            if 1000 <= val <= 9999:
                tps[n_tp] = val
...
class SignalParser:
    ...
    def _parse_trade(self, text: str, timestamp: Optional[datetime] = None) -> Optional[TradeSignal]:
        ...
        # Extract entry: single price first (inline BUY/SELL 4558),
        # then range — IMPORTANT: range AFTER inline to avoid TP lines
        # being captured as range (e.g. "1  4550" from "TP1  4550")
        ...
        if entry_price is not None:
            ...
            if zone:
                ...
                # Sanity check: both bounds must be plausible prices
                if zone_low < 100 or zone_high < 100:
                    return None
            else:
                return None
