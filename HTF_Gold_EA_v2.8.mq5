...
input int               InpST1_Period       = 6;            // [ST1] Période ATR
...
input double            InpRR_Ratio         = 2.3;           // [ATR/PTS] Risk:Reward TP
...
input int               InpMaxMinutes       = 35;            // Fermer après X minutes
...
input int               InpW1_End           = 10;            // [W1] Heure fin London
input bool              InpUseWindow2       = true;          // Fenêtre 2 active (NY)
input int               InpW2_Start         = 12;            // [W2] Heure début NY
input int               InpW2_End           = 19;            // [W2] Heure fin NY
...
int OnInit()
{
   ...
   PrintFormat("HTF Gold EA v2.8 [MER-L1] | %s | H1: %s%s | SL/TP: %s | W1:%d-%d W2:%d-%d",
               symbol, filterName,
               (InpH1FilterType==H1_FILTER_EMA && InpH1_ConfirmH1Bar) ? "+ConfirmH1[0]" : "",
               sltpName,
               InpW1_Start, InpW1_End, InpW2_Start, InpW2_End);
...
bool IsTradeHour()
{
   ...
   // Fenêtre 1 : London (défaut 07:00-10:00)
   bool inW1 = InpUseWindow1 && (h >= InpW1_Start && h < InpW1_End);

   // Fenêtre 2 : NY (défaut 12:00-19:00)
