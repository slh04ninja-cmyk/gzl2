...
//|          FIX    : Handles SuperTrend créés dans OnInit()         |
//+------------------------------------------------------------------+
#property copyright "HTF Gold EA v3.2 - MER Layer 1+2 + ADX + SpreadSL + Risk% + ST Fix"
...
input int               InpST2_Period       = 10;                   // [ST2] Période ATR
...
input int               InpVSP_LookBack     = 8;                    // Chercher spike dans X bougies passées
...
// SuperTrend handles (créés une seule fois dans OnInit pour éviter fuite mémoire)
int            h1_atr_st1_handle   = INVALID_HANDLE;
int            h1_atr_st2_handle   = INVALID_HANDLE;
...
int OnInit()
{
   ...
   // SuperTrend H1 handles — créés une seule fois ici (fix fuite mémoire)
   h1_atr_st1_handle = iATR(g_symbol, PERIOD_H1, InpST1_Period);
   h1_atr_st2_handle = iATR(g_symbol, PERIOD_H1, InpST2_Period);
   ...
   if(h1_ema_fast_handle == INVALID_HANDLE || h1_ema_slow_handle == INVALID_HANDLE ||
      ...
      h1_atr_st1_handle  == INVALID_HANDLE || h1_atr_st2_handle  == INVALID_HANDLE ||
   ...
   PrintFormat("HTF Gold EA v3.2 [MER-L1+L2+ADX+SpreadSL] | %s | H1: %s%s | SL/TP: %s | MSB: %s | ADX: %s | SpreadSL: %s",
               g_symbol, filterName,
               (InpH1FilterType==H1_FILTER_EMA && InpH1_ConfirmH1Bar) ? "+ConfirmH1[0]" : "",
               sltpName,
               InpUseMSB ? StringFormat("ON Bars=%d %s", InpMSB_SwingBars, InpMSB_Timing==0?"BEFORE":"AFTER") : "OFF",
               InpUseADX ? StringFormat("ON period=%d min=%.1f", InpADX_Period, InpADX_MinLevel) : "OFF",
               InpUseSpreadSL ? StringFormat("ON buffer=%d pts", InpSpread_Buffer) : "OFF");
...
void OnDeinit(const int reason)
{
   ...
   IndicatorRelease(h1_atr_st1_handle);
   IndicatorRelease(h1_atr_st2_handle);
...
int GetH1Bias_SuperTrend()
{
   ...
   SuperTrendData st1 = CalcSuperTrend(PERIOD_H1, h1_atr_st1_handle, InpST1_Period, InpST1_Multiplier, barsNeeded);
   SuperTrendData st2 = CalcSuperTrend(PERIOD_H1, h1_atr_st2_handle, InpST2_Period, InpST2_Multiplier, barsNeeded);
...
//+------------------------------------------------------------------+
//| CALCUL SUPERTREND — Utilise le handle ATR pré-créé dans OnInit  |
//| FIX v3.2 : plus de création de handle à chaque tick             |
//+------------------------------------------------------------------+
SuperTrendData CalcSuperTrend(ENUM_TIMEFRAMES tf, int atrHandle, int period, double multiplier, int bars)
{
   ...
   if(CopyBuffer(atrHandle, 0, 0, bars, atrBuf)   < bars ||
