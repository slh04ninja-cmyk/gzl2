...
//|    Couche 1 : Velocity Spike (VSP)  — détection impulsion M1    |
//|    Couche 2 : Micro Structure Break (MSB) — confirmation M1     |
//|    Biais    : Double SuperTrend H1  OU  EMA croisée H1          |
//|    SL/TP    : ATR dynamique  OU  Points fixes                   |
//|    SL smart : Structure Swing + ATR  (Stoploss Areas)           |
...
//|------------------------------------------------------------------|
//|  v3.1 (base)                                                     |
//|    - Velocity Spike (VSP) + Micro Structure Break (MSB)         |
//|    - Double SuperTrend H1 + EMA H1 pour le biais                |
//|    - Filtre ADX H1, SpreadSL, StructureSL, Breakeven            |
//|    - Gestion lot fixe ou % risque capital                       |
//|    - Fermeture par temps + filtre horaire London/NY              |
//|                                                                  |
//|  v3.3 (2026-03-29)                                              |
//|    + FILTRE SPIKE COOLDOWN                                       |
//|      Bloque un nouveau spike si un spike de même direction       |
//|      a déjà été détecté dans les X dernières bougies M1.        |
//|      Évite les entrées multiples rapprochées en zone de chop    |
//|      ou lors de news volatiles (ex: 3 SL consécutifs en 14min). |
//|      Params : InpUseCooldown (bool) · InpCooldownBars (int)     |
//|      Reset indépendant par direction (BUY / SELL).              |
//|                                                                  |
//|  v3.4 (2026-03-29)  — NOUVEAU                                   |
//|    + FILTRE VOLUME (Idée 4)                                      |
//|      Valide un spike uniquement si son volume M1 dépasse        |
//|      EMA(volume, N) × seuil. Confirme participation réelle.     |
//|      Params : InpUseVolumeFilter · InpVol_EMAPeriod             |
//|               InpVol_Multiplier                                  |
//|                                                                  |
//|    + GESTION RISQUE ADAPTIVE (Idée 5)                           |
//|      Réduit le % risque après N pertes consécutives par paliers.|
//|      Retour progressif au nominal après des gains consécutifs.  |
//|      Protège le capital en période de drawdown.                 |
//|      Params : InpUseAdaptiveRisk · InpAR_LossThreshold          |
//|               InpAR_ReduceFactor · InpAR_MaxBoost               |
//|                                                                  |
//|    + FILTRE OUVERTURE SESSION (Idée 6)                          |
//|      Bloque les trades pendant les N premières minutes          |
//|      après l'ouverture de chaque session (London / NY).         |
//|      Évite les faux breakouts et spreads larges en ouverture.   |
//|      Param : InpSessionCooldownMin                              |
//+------------------------------------------------------------------+
#property copyright "HTF Gold EA v3.4 - MER L1+L2 + VolFilter + AdaptiveRisk + SessionCooldown"
...
input int               InpADX_Period       = 14;            // Période ADX H1
input double            InpADX_MinLevel     = 20.0;          // ADX minimum (tendance > range)

input group "=== SL MINIMUM SPREAD ==="
input bool              InpUseSpreadSL      = true;          // Activer SL minimum basé sur spread
...
input group "=== SPIKE COOLDOWN ==="
input bool              InpUseCooldown      = true;          // Activer filtre spike cooldown
...
input group "=== FILTRE VOLUME (Idée 4) ==="
input bool              InpUseVolumeFilter  = true;          // Activer filtre volume spike
input int               InpVol_EMAPeriod    = 20;            // Période EMA du volume M1
input double            InpVol_Multiplier   = 1.2;           // Volume spike > EMA × seuil

input group "=== RISQUE ADAPTATIF (Idée 5) ==="
input bool              InpUseAdaptiveRisk  = true;          // Activer gestion risque adaptative
input int               InpAR_LossThreshold = 2;             // Nb pertes consécutives → réduction
input double            InpAR_ReduceFactor  = 0.5;           // Facteur réduction (ex: 0.5 = moitié)
input double            InpAR_MaxBoost      = 1.25;          // Boost max après gains (ex: 1.25 = +25%)
...
// Handle volume M1 pour filtre volume
int            m1_vol_ema_handle   = INVALID_HANDLE;
...
int            h1_adx_handle       = INVALID_HANDLE; // ADX H1
...
int OnInit()
{
   ...
   // Handle EMA volume M1 (filtre volume)
   m1_vol_ema_handle = iMA(g_symbol, PERIOD_M1, InpVol_EMAPeriod, 0, MODE_EMA, VOLUME_TICK);
   ...
   if(h1_ema_fast_handle == INVALID_HANDLE || h1_ema_slow_handle == INVALID_HANDLE ||
      ...
      m1_atr_str_handle  == INVALID_HANDLE || h1_adx_handle      == INVALID_HANDLE ||
      m1_vol_ema_handle  == INVALID_HANDLE)
   {
   ...
   PrintFormat("HTF Gold EA v3.4 [MER-L1+L2] | %s | H1: %s%s | SL/TP: %s | MSB: %s | Cooldown: %s | Vol: %s | AdaptRisk: %s | SessCooldown: %dmin",
               g_symbol, filterName,
               ...
               InpUseCooldown ? StringFormat("ON %d bougies", InpCooldownBars) : "OFF",
               InpUseVolumeFilter ? StringFormat("ON EMA%d×%.1f", InpVol_EMAPeriod, InpVol_Multiplier) : "OFF",
...
void OnDeinit(const int reason)
{
   ...
   IndicatorRelease(m1_vol_ema_handle);
}
...
void OnTick()
{
   ...
   //=================================================================
   // FILTRE ADX H1
   // Ne trader que si ADX H1 > seuil → tendance réelle
   // ADX < seuil = marché en range → VSP peu fiable
   //=================================================================
   if(InpUseADX)
   {
      double adxBuf[];
   ...
   //=================================================================
   // FILTRE SPIKE COOLDOWN
   // Bloque le signal si un spike de même direction a été détecté
   // dans les InpCooldownBars dernières bougies M1.
   // Evite les entrées multiples en rafale sur une zone chaotique.
   // Reset indépendant par direction (BUY / SELL).
   //=================================================================
   if(InpUseCooldown)
   {
      int dirIdx = (signal == 1) ? 0 : 1;
      int barsElapsed = (int)((TimeCurrent() - g_lastSpikeTime[dirIdx]) / 60);
      if(g_lastSpikeTime[dirIdx] > 0 && barsElapsed < InpCooldownBars)
      {
         PrintFormat("[COOLDOWN] Signal %s bloqué — dernier spike il y a %d bougie(s) < seuil %d",
                     (signal == 1 ? "BUY" : "SELL"), barsElapsed, InpCooldownBars);
         return;
      }
   }
   ...
   // Vérifie que le SL est assez large pour ne pas être touché
   // immédiatement par le spread du broker
   // SL min = spread actuel + buffer de sécurité
   //=================================================================
   if(InpUseSpreadSL)
   {
   ...
   if(signal == 1 && !HasOpenTrade(POSITION_TYPE_BUY))
   {
                                ...
                                lotSize, vsp.barIndex)))
      {
         ...
         // Enregistrer timestamp cooldown direction BUY
         if(InpUseCooldown) g_lastSpikeTime[0] = TimeCurrent();
      }
   }
   else if(signal == -1 && !HasOpenTrade(POSITION_TYPE_SELL))
   {
                                 ...
                                 lotSize, vsp.barIndex)))
      {
         ...
         // Enregistrer timestamp cooldown direction SELL
         if(InpUseCooldown) g_lastSpikeTime[1] = TimeCurrent();
      }
   }
}
...
VSP_Result DetectVelocitySpike()
{
   ...
   //=================================================================
   // FILTRE VOLUME (Idée 4)
   // Charger EMA volume et tick volumes si filtre activé
   //=================================================================
   double volEma[]  = {};
   long   tickVol[] = {};
   bool   volReady  = false;
   ...
   {
      ArraySetAsSeries(volEma, true);
      if(CopyBuffer(m1_vol_ema_handle, 0, 0, barsToCheck, volEma) >= barsToCheck)
      {
         ArraySetAsSeries(tickVol, true);
         if(CopyTickVolume(g_symbol, PERIOD_M1, 0, barsToCheck, tickVol) >= barsToCheck)
            volReady = true;
      }
   }
   ...
   for(int i = 2; i <= InpVSP_LookBack + 1; i++)
   {
      ...
      if(bodySize >= threshold)
      {
         //=============================================================
         // VALIDATION VOLUME
         // Si filtre activé et données disponibles :
         // accepter le spike uniquement si volume > EMA × seuil
         //=============================================================
         if(InpUseVolumeFilter && volReady)
         {
            double emaVol = volEma[i];
            double minVol = emaVol * InpVol_Multiplier;
            if(emaVol > 0 && (double)tickVol[i] < minVol)
            {
               PrintFormat("[VOL BLOQUÉ] Spike bar[%d] vol=%d < EMA×%.1f=%.0f → spike sans participation",
                           i, tickVol[i], InpVol_Multiplier, minVol);
               continue; // chercher un autre spike avec volume suffisant
            }
            PrintFormat("[VOL OK] Spike bar[%d] vol=%d >= EMA×%.1f=%.0f",
                        i, tickVol[i], InpVol_Multiplier, minVol);
         }
...
bool IsTradeHour()
{
   ...
   //=================================================================
   // FILTRE OUVERTURE SESSION (Idée 6)
   // Bloquer les N premières minutes après l'ouverture de chaque
   // session pour éviter les faux breakouts et spreads larges.
   //=================================================================
...
//|                                                                  |
//| Surveille le dernier trade fermé par ce bot et met à jour       |
//| les compteurs de séries pour moduler le multiplicateur de risque.|
//|                                                                  |
//| Paliers :                                                        |
//|  0 perte consécutive  → mult = 1.0  (nominal)                  |
//|  >= LossThreshold     → mult = ReduceFactor                     |
//|  >= LossThreshold×2   → mult = ReduceFactor²  (mode survie)    |
//|  gains consécutifs    → remontée progressive jusqu'à MaxBoost   |
//+------------------------------------------------------------------+
void UpdateAdaptiveRisk()
{
   if(!InpUseAdaptiveRisk) return;
   if(InpLotMode == LOT_MODE_FIXED) return; // Pas pertinent en lot fixe

   // Parcourir l'historique pour trouver le dernier deal de ce bot
   ...
   for(int i = totalDeals - 1; i >= 0; i--)
   {
      ...
      else if(profit > 0) // Gain
      {
         g_consecutiveWins++;
         g_consecutiveLosses = 0;
      }
      break; // On ne regarde que le dernier trade
   }

   // Calcul du multiplicateur selon les paliers
   double newMult = 1.0;

   if(g_consecutiveLosses >= InpAR_LossThreshold * 2)
      newMult = InpAR_ReduceFactor * InpAR_ReduceFactor; // Mode survie
   else if(g_consecutiveLosses >= InpAR_LossThreshold)
      newMult = InpAR_ReduceFactor;                       // Réduction 1er palier
   else if(g_consecutiveWins >= 2)
      newMult = MathMin(InpAR_MaxBoost, 1.0 + (g_consecutiveWins - 1) * 0.1); // Boost progressif
...
double CalcLotSize(double slDist)
{
   ...
   // Calcul % risque — modulé par le multiplicateur adaptatif
