...
//
//+------------------------------------------------------------------+
#property copyright "HTF Gold EA v2.5"
...
int OnInit()
{
   ...
   PrintFormat("[HTF EA v2.5] Démarré | %s | H1: %s | M1: VSP(ATR%d×%.1f LB=%d%s) | %s",
               symbol, h1Method,
...
int GetH1Bias_EMA()
{
   ...
   int bias = 0;
   if(emaFast[1] > emaSlow[1]) bias =  1;
   if(emaFast[1] < emaSlow[1]) bias = -1;

   PrintFormat("[DEBUG H1] EMA_Fast[1]=%.3f EMA_Slow[1]=%.3f → bias=%d(%s)",
               emaFast[1], emaSlow[1], bias,
               bias==1?"BULL":bias==-1?"BEAR":"NEUTRE");

   if(bias == 0) return 0;

   // Condition 2 : confirmation prix actuel vs Open H1[0]
   if(InpH1_ConfirmCurrent)
   {
      ...
      PrintFormat("[DEBUG H1] CurrentBid=%.3f OpenH1[0]=%.3f → %s",
                  currentBid, openH1,
                  (bias==1 && currentBid>=openH1)||(bias==-1 && currentBid<=openH1)
                  ? "CONFIRMÉ ✅" : "BLOQUÉ ❌");
...
VSP_Result DetectVelocitySpike(int h1Bias)
{
   ...
   for(int i = 2; i <= InpVSP_LookBack + 1; i++)
   {
      ...
      if(bodySize >= atrVal * InpVSP_Spike_Multi)
      {
         ...
         PrintFormat("[DEBUG VSP] bar[%d] body=%.3f ATR=%.3f thresh=%.3f spikeDir=%d needed=%d → %s",
                     i, bodySize, atrVal, atrVal*InpVSP_Spike_Multi,
                     spikeDir, spikeNeeded,
                     spikeDir==spikeNeeded ? "ACCEPTÉ ✅" : "REJETÉ ❌");
              ...
              double slDist, double tpDist, VSP_Result &vsp)
{
   ...
   PrintFormat("[HTF EA v2.5][%s] %s @ %.3f | SL=%.3f(%.0fpts/%.2f$) | TP=%.3f(%.0fpts/%.2f$) | spike bar[%d] body=%.3f",
