...
class MT5Bridge:
    ...
    def place_market_order(
        ...
        log.info(f"[DEBUG] prix={price} filling_mode={sym.trade_fill_mode}")
        otype = (
...
def execute_signal(signal: dict, bridge: MT5Bridge, manager, tracker):
    ...
    if in_zone:
        log.info(f"[DEBUG] Appel place_market_order lot={LOT_SIZE} tp={tp2} sym={sym_info.name}")
        try:
            t1 = bridge.place_market_order(signal, LOT_SIZE, tp=tp2)
        except Exception as e:
            log.error(f"[DEBUG] EXCEPTION: {e}")
            t1 = None
        log.info(f"[DEBUG] Résultat: t1={t1}")
        if t1:
            tickets.append(
                {
                    "ticket": t1,
