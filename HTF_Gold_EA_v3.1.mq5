...
//+------------------------------------------------------------------+
#property copyright "HTF Gold EA v3.1 - MER Layer 1+2 + SAR"
...
input group "=== COUCHE 3 : SHADOW ABSORPTION RATIO (SAR) ==="
input bool              InpUseSAR           = true;          // Activer SAR
input double            InpSAR_MinRatio     = 0.2;           // Ratio mèche/corps minimum
input bool              InpSAR_DynLot       = false;         // Activer lot dynamique selon SAR
input double            InpSAR_Lot_Low      = 0.01;          // Lot si SAR faible  (< 0.3)
input double            InpSAR_Lot_Mid      = 0.02;          // Lot si SAR moyen   (0.3 → 0.6)
input double            InpSAR_Lot_High     = 0.03;          // Lot si SAR fort    (> 0.6)
...
int OnInit()
{
   ...
   PrintFormat("HTF Gold EA v3.1 [MER-L1+L2+SAR] | %s | H1: %s%s | SL/TP: %s | MSB: %s | SAR: %s",
               symbol, filterName,
               (InpH1FilterType==H1_FILTER_EMA && InpH1_ConfirmH1Bar) ? "+ConfirmH1[0]" : "",
               sltpName,
               InpUseMSB ? StringFormat("ON Bars=%d %s",
                           InpMSB_SwingBars,
                           InpMSB_Timing==MSB_TIMING_BEFORE?"BEFORE":"AFTER") : "OFF",
               InpUseSAR ? StringFormat("ON MinRatio=%.2f DynLot=%s",
                           InpSAR_MinRatio, InpSAR_DynLot?"ON":"OFF") : "OFF");
...
void OnTick()
{
   ...
   //=================================================================
   // COUCHE 3 : SHADOW ABSORPTION RATIO (SAR)
   // Vérifie que la bougie spike montre une absorption
   // (mèche dans direction opposée au spike / corps)
   // Et ajuste le lot si lot dynamique activé
   //=================================================================
   double lotSize = InpLotSize; // lot par défaut
   if(InpUseSAR)
   {
      double sar = CalcSAR(vsp.barIndex, vsp.direction);
      PrintFormat("[SAR] Ratio=%.3f Seuil=%.2f → %s",
                  sar, InpSAR_MinRatio,
                  sar >= InpSAR_MinRatio ? "ACCEPTÉ ✅" : "REJETÉ ❌");

      if(sar < InpSAR_MinRatio)
      {
         PrintFormat("[SAR BLOQUÉ] Absorption insuffisante → Marubozu rejeté");
         return;
      }

      // Lot dynamique selon force de l'absorption
      if(InpSAR_DynLot)
      {
         if     (sar > 0.6) lotSize = InpSAR_Lot_High;
         else if(sar > 0.3) lotSize = InpSAR_Lot_Mid;
         else               lotSize = InpSAR_Lot_Low;
         PrintFormat("[SAR] Lot dynamique = %.2f (SAR=%.3f)", lotSize, sar);
      }
   }
   ...
   if(signal == 1 && !HasOpenTrade(POSITION_TYPE_BUY))
   {
      ...
      if(trade.Buy(lotSize, symbol, ask, sl, tp,
                   StringFormat("MER-L1+SAR|BUY|SAR=%.2f|spike@%d",
                                InpUseSAR ? CalcSAR(vsp.barIndex, vsp.direction) : 0.0,
                                vsp.barIndex)))
   ...
   else if(signal == -1 && !HasOpenTrade(POSITION_TYPE_SELL))
   {
      ...
      if(trade.Sell(lotSize, symbol, bid, sl, tp,
                    StringFormat("MER-L1+SAR|SELL|SAR=%.2f|spike@%d",
                                 InpUseSAR ? CalcSAR(vsp.barIndex, vsp.direction) : 0.0,
                                 vsp.barIndex)))
...
//| COUCHE 3 — SHADOW ABSORPTION RATIO (SAR)                       |
//|                                                                  |
//| Calcule le ratio mèche/corps de la bougie spike                 |
//|                                                                  |
//| Spike BAISSIER (-1) → mèche basse / corps                      |
//|   Mèche basse = Min(Open,Close) - Low                          |
//|   → Les acheteurs ont résisté en bas = absorption ✅            |
//|                                                                  |
//| Spike HAUSSIER (+1) → mèche haute / corps                      |
//|   Mèche haute = High - Max(Open,Close)                         |
//|   → Les vendeurs ont résisté en haut = absorption ✅            |
//|                                                                  |
//| SAR = 0.0 → Marubozu (pas d'absorption)                        |
//| SAR = 1.0 → Mèche = Corps (absorption forte)                   |
//| SAR > 1.0 → Mèche > Corps (absorption très forte)              |
//+------------------------------------------------------------------+
double CalcSAR(int barIndex, int spikeDir)
{
   double openBuf[], closeBuf[], highBuf[], lowBuf[];
   ArraySetAsSeries(openBuf,  true);
   ArraySetAsSeries(closeBuf, true);
   ArraySetAsSeries(highBuf,  true);
   ArraySetAsSeries(lowBuf,   true);

   int barsNeeded = barIndex + 2;
   if(CopyOpen (symbol, PERIOD_M1, 0, barsNeeded, openBuf)  < barsNeeded) return 0.0;
   if(CopyClose(symbol, PERIOD_M1, 0, barsNeeded, closeBuf) < barsNeeded) return 0.0;
   if(CopyHigh (symbol, PERIOD_M1, 0, barsNeeded, highBuf)  < barsNeeded) return 0.0;
   if(CopyLow  (symbol, PERIOD_M1, 0, barsNeeded, lowBuf)   < barsNeeded) return 0.0;

   double o = openBuf[barIndex];
   double c = closeBuf[barIndex];
   double h = highBuf[barIndex];
   double l = lowBuf[barIndex];

   double body = MathAbs(c - o);
   if(body <= 0) return 0.0; // Doji → pas de calcul

   double shadow = 0.0;

   if(spikeDir == -1) // Spike BAISSIER → mèche BASSE
      shadow = MathMin(o, c) - l;
   else               // Spike HAUSSIER → mèche HAUTE
      shadow = h - MathMax(o, c);
