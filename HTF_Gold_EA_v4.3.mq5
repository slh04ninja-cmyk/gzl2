...
//+------------------------------------------------------------------+
#property copyright "HTF Gold EA v4.3 - MER L1+L2 + AdaptRisk + SessionCooldown + Regime + CircuitBreaker"
...
input group "=== v4.3 : FILTRE RÉGIME DE MARCHÉ ==="
input bool              InpUseRegime        = true;          // Activer filtre régime de marché
...
input group "=== v4.3 : CIRCUIT BREAKER DRAWDOWN ==="
input bool              InpUseCircuitBreaker = true;          // Activer circuit breaker
...
// --- v4.3 : Résultat du filtre régime
struct RegimeResult
{
...
int OnInit()
{
   ...
   // --- v4.3 : Initialiser Circuit Breaker ---
   ...
   PrintFormat("HTF Gold EA v4.3 | %s | Bias TF=%dmin (%s) | DoubleST(%d×%.1f / %d×%.1f) | SL/TP: %s | MSB: %s | Cooldown: %d bougies | AdaptRisk: %s | SessCooldown: %dmin | Retournement: %s",
               g_symbol,
...
void OnTick()
{
   ...
   // --- v4.3 : Circuit Breaker (vérification haute priorité) ---
   if(InpUseCircuitBreaker)
   {
      ...
      if(IsCircuitBreakerActive())
      {
         if(!InpCB_AlertOnly)
            ...
            PrintFormat("[CB ALERT] Circuit breaker actif mais mode alerte → trading autorisé avec avertissement");
      }
   }
   ...
   if(InpUseRegime)
   {
      ...
      {
         PrintFormat("[RÉGIME BLOQUÉ] %s | ADX=%.1f | VolRatio=%.2f | Eff=%.2f → pas de trade",
                     regime.description, regime.adxValue, regime.volatilityRatio, regime.efficiencyRatio);
         return;
      }
      ...
      if(regime.regime == REGIME_WEAK_TREND)
      {
         ...
         PrintFormat("[RÉGIME RÉDUIT] %s | ADX=%.1f | Eff=%.2f → risque ×%.2f",
                     regime.description, regime.adxValue, regime.efficiencyRatio, effectiveRiskFactor);
      }
      else
      {
         PrintFormat("[RÉGIME OK] %s | ADX=%.1f | VolRatio=%.2f | Eff=%.2f",
                     regime.description, regime.adxValue, regime.volatilityRatio, regime.efficiencyRatio);
      }
   }
   ...
   if(InpVSP_NeedConfirm)
   {
      ...
      if(signal ==  1 && closeC <= openC) { PrintFormat("[CONFIRM BLOQUÉ] Biais=BULL confirmation baissière → BUY annulé"); return; }
      if(signal == -1 && closeC >= openC) { PrintFormat("[CONFIRM BLOQUÉ] Biais=BEAR confirmation haussière → SELL annulé"); return; }
   }
   ...
   if(InpUseMSB)
   {
      ...
      if(!msb.detected) { PrintFormat("[MSB BLOQUÉ] Pas de structure M1 cassée dans direction %d", signal); return; }
   ...
   if(InpUseSpreadSL)
   {
      ...
      if(slDist < slMinDist)
      {
         PrintFormat("[SPREAD SL BLOQUÉ] SL=%.1f pts < min=%.1f pts → trade rejeté",
                     slDist/g_point, slMinDist/g_point);
         return;
      }
   }
   ...
   if(signal == 1 && !HasOpenTrade(POSITION_TYPE_BUY))
   {
      ...
      if(trade.Buy(lotSize, g_symbol, ask, sl, tp,
                   StringFormat("HTF-v4.3|BUY|%s|lot=%.2f|spike@%d",
   ...
   else if(signal == -1 && !HasOpenTrade(POSITION_TYPE_SELL))
   {
      ...
      if(trade.Sell(lotSize, g_symbol, bid, sl, tp,
                    StringFormat("HTF-v4.3|SELL|%s|lot=%.2f|spike@%d",
                                 (InpSLTP_Mode==SLTP_MODE_ATR?"ATR":"PTS"), lotSize, vsp.barIndex)))
         PrintSignal("SELL", bid, sl, tp, MathAbs(bid-sl), MathAbs(bid-tp), vsp);
   }
}
//+------------------------------------------------------------------+
//| v4.3 : DÉTECTION RÉGIME DE MARCHÉ                               |
//+------------------------------------------------------------------+
RegimeResult DetectMarketRegime()
{
   ...
   result.adxValue         = 0;
   result.volatilityRatio  = 1.0;
   result.efficiencyRatio  = 0.5;
   result.description      = "";

   // --- 1. ADX sur TF de biais ---
   double adxBuf[];
   ArraySetAsSeries(adxBuf, true);
   if(CopyBuffer(bias_adx_handle, 0, 0, 3, adxBuf) < 3)
   {
      result.description = "ADX indisponible → fallback STRONG_TREND";
      return result;
   }
   result.adxValue = adxBuf[1];

   // --- 2. Ratio de volatilité (ATR actuel / ATR moyen) sur H1 ---
   int atrH1Handle = iATR(g_symbol, PERIOD_H1, InpRegime_ATRPeriod);
   if(atrH1Handle == INVALID_HANDLE)
   {
      result.description = "ATR H1 indisponible → fallback STRONG_TREND";
      return result;
   }
   ...
   {
      result.description = "ATR H1 buffer insuffisant → fallback STRONG_TREND";
      return result;
   }
   ...
   if(result.volatilityRatio > InpRegime_VolRatioMax)
   {
      ...
      result.description = StringFormat("VOLATILE | ATR=%.2f / AvgATR=%.2f (ratio=%.2f > seuil=%.1f)",
                                        currentATR, avgATR, result.volatilityRatio, InpRegime_VolRatioMax);
      return result;
   }
   ...
   result.description = StringFormat("WEAK_TREND | ADX=%.1f | Eff=%.2f | VolRatio=%.2f",
                                     result.adxValue, result.efficiencyRatio, result.volatilityRatio);
   return result;
}

//+------------------------------------------------------------------+
//| v4.3 : CIRCUIT BREAKER — Mise à jour des compteurs              |
//+------------------------------------------------------------------+
void UpdateCircuitBreaker()
{
   ...
   for(int i = totalDeals - 1; i >= 0; i--)
   {
      ...
      {
         g_cbConsecLosses++;
         PrintFormat("[CB TRACK] Perte consécutive #%d | Profit=%.2f", g_cbConsecLosses, profit);
      }
      else if(profit > 0)
      {
         if(g_cbConsecLosses > 0)
            PrintFormat("[CB TRACK] Gain → reset compteur pertes (était à %d)", g_cbConsecLosses);
         g_cbConsecLosses = 0;
      }
      break;
   }
...
//+------------------------------------------------------------------+
//| v4.3 : CIRCUIT BREAKER — Vérification active/inactive           |
//+------------------------------------------------------------------+
bool IsCircuitBreakerActive()
{
   if(!InpUseCircuitBreaker) return false;

   // Vérifier si on est encore en période de cooldown
   if(g_circuitBreakerTime > 0)
   {
      ...
      if(elapsedMin < InpCB_CooldownMinutes)
      {
         if(!g_cbActive)
         {
            PrintFormat("[CIRCUIT BREAKER ACTIF] %s | Reprise dans %d min",
                        g_cbReason, InpCB_CooldownMinutes - elapsedMin);
            g_cbActive = true;
         }
         return true;
      }
      ...
      {
         PrintFormat("[CIRCUIT BREAKER] Cooldown terminé → reprise du trading | PeakEquity reset");
         g_circuitBreakerTime = 0;
         g_cbActive = false;
         g_cbReason = "";
   ...
   // Vérifier 1 : Drawdown depuis le pic d'équité
   double equity = AccountInfoDouble(ACCOUNT_EQUITY);
   double ddPct  = 0;
   if(g_peakEquity > 0)
   ...
   if(ddPct >= InpCB_MaxDrawdownPct)
   {
      ...
      g_cbReason = StringFormat("DRAWDOWN %.2f%% ≥ seuil %.2f%% (pic=%.2f$ → equity=%.2f$)",
                                ddPct, InpCB_MaxDrawdownPct, g_peakEquity, equity);
      g_cbActive = true;
      PrintFormat("[CIRCUIT BREAKER DÉCLENCHÉ] %s → TRADING STOPPÉ pendant %d min",
                  g_cbReason, InpCB_CooldownMinutes);
      return true;
   }

   // Vérifier 2 : Nombre de pertes consécutives
   if(g_cbConsecLosses >= InpCB_LossStreakLimit)
   {
      g_circuitBreakerTime = TimeCurrent();
      g_cbReason = StringFormat("Pertes consécutives: %d ≥ limite: %d",
                                g_cbConsecLosses, InpCB_LossStreakLimit);
      g_cbActive = true;
      PrintFormat("[CIRCUIT BREAKER DÉCLENCHÉ] %s → TRADING STOPPÉ pendant %d min",
                  g_cbReason, InpCB_CooldownMinutes);
      return true;
                 ...
                 double slDist, double tpDist, VSP_Result &vsp)
{
   ...
   double lotUsed = (InpLotMode == LOT_MODE_FIXED) ? InpLotSize : CalcLotSize(slDist, 1.0);
   ...
   PrintFormat("[MER-L1][%s][LOT:%s=%.2f][REGIME:%s] %s @ %.5f | SL=%.5f(%.0fpts/%.2f$) | TP=%.5f(%.0fpts/%.2f$) | Spike@%d",
               mode, lotMode, lotUsed, g_regimeDescription, dir, price,
...
//+------------------------------------------------------------------+
//| CALCUL LOT SELON MODE (v4.3 : avec facteur de risque régime)   |
//+------------------------------------------------------------------+
double CalcLotSize(double slDist, double regimeRiskFactor = 1.0)
{
   ...
   PrintFormat("[LOT%%] Capital=%.2f$ Risk=%.1f%%×Adapt(%.2f)×Regime(%.2f)=%.2f$ SL=%.0fpts → Lot=%.2f",
               capital, InpRiskPercent, adaptiveMult, regimeRiskFactor,
               effectiveRisk, slPoints, lot);
