...
input int               InpSL_Points        = 500;           // [PTS] SL en points (500 pts = 5$)
input int               InpTP_Points        = 700;           // [PTS] TP en points (700 pts = 7$)
...
int OnInit()
{
   ...
   string sltpName = (InpSLTP_Mode == SLTP_MODE_ATR) ?
                     ...
                     StringFormat("Points | SL=%d pts (%.2f$) | TP=%d pts (%.2f$)",
                                  InpSL_Points, InpSL_Points * InpLotSize / 10.0,
                                  InpTP_Points, InpTP_Points * InpLotSize / 10.0);
...
void OnTick()
{
   ...
   // Spike OPPOSÉ au biais = épuisement contre-tendance
   if(vsp.direction == h1Bias) return;

   // Signal = direction biais H1
   int signal = h1Bias;
                 ...
                 double slDist, double tpDist, VSP_Result &vsp)
{
   ...
   double slUSD   = slPts * InpLotSize / 100.0;
   double tpUSD   = tpPts * InpLotSize / 100.0;
