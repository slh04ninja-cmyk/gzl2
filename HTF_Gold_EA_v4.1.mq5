...
int OnInit()
{
   ...
   PrintFormat("HTF Gold EA v4.1 | %s | H1: DoubleST(%d×%.1f / %d×%.1f) | SL/TP: %s | MSB: %s | Cooldown intégré: %d bougies | AdaptRisk: %s | SessCooldown: %dmin | Retournement: %s",
               g_symbol,
...
bool IsPullbackStructure(int signal, int spikeBarIndex, double &atrBuf[])
{
   ...
   {
      // Chercher le dernier swing low significatif AVANT le spike
      // (zone entre spikeBarIndex+1 et spikeBarIndex+InpRT_LookBack)
      int refStart = spikeBarIndex + 1;
