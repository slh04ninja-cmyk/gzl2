...
//+------------------------------------------------------------------+
#property copyright "HTF Gold EA v2.9b - MER Layer 1+2 + Risk%"
...
void OnTick()
{
   ...
   if(signal == 1 && !HasOpenTrade(POSITION_TYPE_BUY))
   {
      ...
      if(trade.Buy(lotSize, symbol, ask, sl, tp,
                   StringFormat("MER-L1|BUY|%s|lot=%.2f|spike@%d",
   ...
   else if(signal == -1 && !HasOpenTrade(POSITION_TYPE_SELL))
   {
      ...
      if(trade.Sell(lotSize, symbol, bid, sl, tp,
                    StringFormat("MER-L1|SELL|%s|lot=%.2f|spike@%d",
                                 (InpSLTP_Mode==SLTP_MODE_ATR?"ATR":"PTS"),
                                 lotSize, vsp.barIndex)))
...
double CalcLotSize(double slDist)
{
   ...
   // Valeur d'1 point pour 1 lot sur XAUUSDm = 0.1$
