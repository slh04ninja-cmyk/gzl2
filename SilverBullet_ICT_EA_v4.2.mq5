...
// Handles EMA pre-crees (correction backtest : ne pas creer dans les fonctions)
int h_MacroEMA1   = INVALID_HANDLE;
int h_MacroEMA2   = INVALID_HANDLE;
int h_BiaisEMA1   = INVALID_HANDLE;
int h_BiaisEMA2   = INVALID_HANDLE;
int h_ConfirmEMA1 = INVALID_HANDLE;
int h_ConfirmEMA2 = INVALID_HANDLE;
...
int OnInit()
{
   ...
   // Pre-creation handles EMA (OBLIGATOIRE pour backtest)
   // Creer une seule fois dans OnInit evite le probleme de buffer vide
   h_BiaisEMA1   = iMA(_Symbol, BiaisTF,   Biais_EMA1_Period,   0, MODE_EMA, PRICE_CLOSE);
   h_BiaisEMA2   = iMA(_Symbol, BiaisTF,   Biais_EMA2_Period,   0, MODE_EMA, PRICE_CLOSE);
   h_ConfirmEMA1 = iMA(_Symbol, ConfirmTF, Confirm_EMA1_Period, 0, MODE_EMA, PRICE_CLOSE);
   h_ConfirmEMA2 = iMA(_Symbol, ConfirmTF, Confirm_EMA2_Period, 0, MODE_EMA, PRICE_CLOSE);

   if(h_BiaisEMA1==INVALID_HANDLE || h_BiaisEMA2==INVALID_HANDLE ||
      h_ConfirmEMA1==INVALID_HANDLE || h_ConfirmEMA2==INVALID_HANDLE) {
      Print("[ERR] Handles EMA invalides");
      return INIT_FAILED;
   }

   // Macro EMA (seulement si filtre actif)
   if(UseMacroFilter) {
      h_MacroEMA1 = iMA(_Symbol, MacroTF, Macro_EMA1_Period, 0, MODE_EMA, PRICE_CLOSE);
      if(h_MacroEMA1 == INVALID_HANDLE) {
         Print("[ERR] Handle Macro EMA1 invalide");
         return INIT_FAILED;
      }
      if(Macro_EMA2_Period > 0) {
         h_MacroEMA2 = iMA(_Symbol, MacroTF, Macro_EMA2_Period, 0, MODE_EMA, PRICE_CLOSE);
         if(h_MacroEMA2 == INVALID_HANDLE) {
            Print("[ERR] Handle Macro EMA2 invalide");
            return INIT_FAILED;
         }
      }
   }
...
void OnDeinit(const int reason)
{
   ...
   if(h_BiaisEMA1    != INVALID_HANDLE) IndicatorRelease(h_BiaisEMA1);
   if(h_BiaisEMA2    != INVALID_HANDLE) IndicatorRelease(h_BiaisEMA2);
   if(h_ConfirmEMA1  != INVALID_HANDLE) IndicatorRelease(h_ConfirmEMA1);
   if(h_ConfirmEMA2  != INVALID_HANDLE) IndicatorRelease(h_ConfirmEMA2);
   if(h_MacroEMA1    != INVALID_HANDLE) IndicatorRelease(h_MacroEMA1);
   if(h_MacroEMA2    != INVALID_HANDLE) IndicatorRelease(h_MacroEMA2);
   Comment("");
   PrintDebugStats();
}
...
void OnTick()
{
   ...
   if(g_DailyTrades >= MaxDailyTrades) { dbg_NoMaxTrade++; return; }
   if(HasOpenPosition())               return;
   if(IsSpreadTooHigh())               { dbg_NoSpread++; return; }
...
{
   if(UseBiasEMA) {
      double ema1 = ReadEMA(h_MacroEMA1);
      if(ema1 == 0) return 0;
      if(Macro_EMA2_Period > 0) {
         double ema2 = ReadEMA(h_MacroEMA2);
         if(ema2 == 0) return 0;
...
{
   if(UseBiasEMA) {
      double ema1 = ReadEMA(h_BiaisEMA1);
      double ema2 = ReadEMA(h_BiaisEMA2);
...
{
   if(UseBiasEMA) {
      double ema1 = ReadEMA(h_ConfirmEMA1);
      double ema2 = ReadEMA(h_ConfirmEMA2);
...
//| OUTILS BIAIS                                                    |
//+------------------------------------------------------------------+
// Lecture EMA via handle pre-cree (fiable en backtest)
double ReadEMA(int handle, int shift = 1)
{
...
// Fallback : creation handle a la volee (moins fiable en backtest)
...
{
   Print("=== STATS BACKTEST Silver Bullet ICT v4.2 ===");
   Print(StringFormat("Bougies hors fenetre : %d", dbg_NoWindow));
   Print(StringFormat("MaxTrades bloque     : %d", dbg_NoMaxTrade));
   Print(StringFormat("Spread bloque        : %d (MaxSpread=%d)", dbg_NoSpread, MaxSpread));
...
void PrintConfig()
{
   ...
   Print("=== Sessions heure broker (GMT+2 Exness) ===");
