...
int OnInit()
  {
   ...
   // Auto-detection du filling mode compatible avec le broker
   ENUM_ORDER_TYPE_FILLING fillingMode = ORDER_FILLING_FOK;
   uint filling = (uint)SymbolInfoInteger(_Symbol, SYMBOL_FILLING_MODE);
   if((filling & SYMBOL_FILLING_IOC) != 0)
      fillingMode = ORDER_FILLING_IOC;
   else if((filling & SYMBOL_FILLING_BOC) != 0)
      fillingMode = ORDER_FILLING_BOC;
...
void OnTick()
  {
   //================================================================
   // Enregistrer le tick (tableau circulaire)
   // Le calcul PI se fait seulement a la nouvelle bougie
   // → evite O(n²) sur chaque tick
   //================================================================
   EnregistrerTick();

   //================================================================
   // Nouvelle barre M1 seulement
   //================================================================
   ...
   //================================================================
   // Calculer PI une seule fois par bougie (pas a chaque tick)
   //================================================================
   g_pressure_current = CalculerPressureIndex();
   if(InpShowPressure)
      AfficherPressure(g_pressure_current, (MQLInfoInteger(MQL_TESTER)==1));
   ...
   //================================================================
   // COOLDOWN
   // Fix : initialiser g_lastTime a la premiere barre valide
   //================================================================
   ...
   if(barsPassed < InpCooldown) return;
   ...
   //================================================================
   // SIGNAL
   // Live    : Pressure Index (spread reel)
   ...
   if(!isBacktest && g_tick_count >= InpTickCount / 2)
     ...
     {
      // BACKTEST : direction bougie + confirmation + impulsion
   ...
   Print(StringFormat("[SIGNAL] %s | C1=%.3f O1=%.3f C2=%.3f | ATR=%.3f | BT=%s | PI=%.3f",
         signalBUY?"BUY":"SELL", close1, open1, close2, atrVal,
         isBacktest?"OUI":"NON", g_pressure_current));

   //================================================================
   // CALCUL SL / TP via ATR dynamique
   // ATR periode 5 sur M1 = reactivite maximale
   // SL = ATR x InpSL_ATR  |  TP = ATR x InpTP_ATR
   // (atrVal deja calcule et valide ci-dessus)
   //================================================================
      ...
      if(trade.Buy(lotActuel, _Symbol, ask, sl, tp, "HFT_BUY"))
        {
         ...
         Print(StringFormat("Erreur BUY code=%d : %s | lot=%.2f sl=%.3f tp=%.3f",
      ...
      if(trade.Sell(lotActuel, _Symbol, bid, sl, tp, "HFT_SELL"))
        {
         ...
         Print(StringFormat("Erreur SELL code=%d : %s | lot=%.2f sl=%.3f tp=%.3f",
               trade.ResultRetcode(), trade.ResultRetcodeDescription(),
               lotActuel, sl, tp));
     }
  }
