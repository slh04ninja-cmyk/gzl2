//+------------------------------------------------------------------+
//|                                           HFT_Gold_EA.mq5        |
//|                    Expert Advisor HFT — Version 1.1              |
//|                                                                   |
...
//|    8 : Kelly Adaptatif                                           |
//+------------------------------------------------------------------+
#property copyright "HFT_Gold_EA v1.1"
...
void OnTick()
  {
   ...
   //================================================================
   if(InpShowPressure)
      AfficherPressure(g_pressure_current);
   ...
   //================================================================
   if(g_tick_count < InpTickCount / 2) return;  // pas assez de donnees

   bool   signalBUY  = (g_pressure_current >  InpPressureMin);
   bool   signalSELL = (g_pressure_current < -InpPressureMin);
   bool   signalFort = (MathAbs(g_pressure_current) >= InpPressureStrong);
...
//+------------------------------------------------------------------+
void AfficherPressure(double pi)
  {
   color   clr;
   string  txt;

   if(pi > InpPressureStrong)
     { clr = clrLime;   txt = StringFormat("PRESSURE : +%.3f  [BUY FORT]",  pi); }
   else if(pi > InpPressureMin)
     { clr = clrGreen;  txt = StringFormat("PRESSURE : +%.3f  [BUY]",       pi); }
   else if(pi < -InpPressureStrong)
     { clr = clrRed;    txt = StringFormat("PRESSURE : %.3f  [SELL FORT]",  pi); }
   else if(pi < -InpPressureMin)
     { clr = clrOrange; txt = StringFormat("PRESSURE : %.3f  [SELL]",       pi); }
   else
     { clr = clrGray;   txt = StringFormat("PRESSURE : %.3f  [neutre]",     pi); }
