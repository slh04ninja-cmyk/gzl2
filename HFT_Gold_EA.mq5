//+------------------------------------------------------------------+
//|                                           HFT_Gold_EA.mq5        |
//|                    Expert Advisor HFT — Version 2.1              |
//|                                                                   |
...
//+------------------------------------------------------------------+
#property copyright "HFT_Gold_EA v2.1"
...
void OnTick()
  {
   ...
   if(!isBacktest && g_tick_count >= InpTickCount / 2)
   ...
   else
     {
      ...
      // Amplitude >= 50% ATR : evite les micro-bougies sans interet
      // Amplitude >= 80% ATR : signal fort
      double ampl = MathAbs(close1 - open1);
      bool   impulsion = (ampl >= atrVal * 0.50);  // filtre qualite
      signalBUY   = (close1 > open1) && (close1 > close2) && impulsion;
      signalSELL  = (close1 < open1) && (close1 < close2) && impulsion;
      signalFort  = (ampl >= atrVal * 0.80);
     }

   if(!signalBUY && !signalSELL) return;

   // SL/TP ATR dynamique
   // Note : lot jamais double en backtest (spread synthetique fausse PI)
