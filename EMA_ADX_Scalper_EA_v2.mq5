...
//+------------------------------------------------------------------+
#property copyright "EMA_ADX_Scalper_EA v2.5"
...
input int    InpBE_Offset    = 2000;   // BE pose a X pips de l entree (lock profit)
input int    InpTrailStep    = 1000;   // Etape trail en pips apres le BE
      ...
      if(g_t1_closed_profit)
        {
         ...
         //-----------------------------------------------------------
         // TRAIL PAR ETAPES — calcule depuis l ENTREE (pas depuis BE)
         //
         // SL final = max(BE_level, Trail_level)
         //
         // BE_level   = entree + BE_Offset (minimum garanti fixe)
         // Trail_level = entree + etapes × TrailStep
         //   etapes = floor(profit / TrailStep)
         //
         // Exemple BE=2000 TrailStep=1000 :
         //   profit 1500 → trail=1000, SL=max(2000,1000)=2000  ← BE protege
         //   profit 2500 → trail=2000, SL=max(2000,2000)=2000
         //   profit 3500 → trail=3000, SL=max(2000,3000)=3000  ← trail prend le relai
         //   profit 4500 → trail=4000, SL=max(2000,4000)=4000
         //-----------------------------------------------------------
         int    etapes        = (int)MathFloor(profit_dist / trail_dist);
         double trail_sl_dist = etapes * trail_dist;

         // SL = le plus grand entre BE_level et Trail_level
         double best_sl_dist = MathMax(be_offset, trail_sl_dist);

         if(posType == POSITION_TYPE_BUY)
           {
            double trail_sl = NormalizeDouble(openPrice + best_sl_dist, _Digits);
            if(trail_sl > newSL)
               newSL = trail_sl;
           }
         else
