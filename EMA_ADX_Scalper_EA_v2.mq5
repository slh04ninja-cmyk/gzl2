...
//+------------------------------------------------------------------+
#property copyright "EMA_ADX_Scalper_EA v2.3"
...
double   g_t2_open_price    = 0;      // prix entree T2 (pour detection TP par prix)
...
//+------------------------------------------------------------------+
//  VERIFIER SI T1 A ETE FERME (compatible backtesting ET live)
//  Methode : detection par le prix au lieu de l historique
//  → Fonctionne en backtesting tick par tick
//+------------------------------------------------------------------+
void VerifierT1Ferme()
  {
   if(g_t1_closed_profit) return;    // BE deja declenche
   if(g_t1_open_time == 0) return;   // aucun groupe ouvert
   if(g_t2_open_price <= 0) return;  // prix entree T2 inconnu

   double pip_size  = 0.01;
   double tp1_dist  = InpTP_Pips * pip_size;
   double sl_dist   = InpSL_Pips * pip_size;
   ...
   if(!t2_open) { g_t1_open_time = 0; return; }  // T2 ferme exterieurement

   // Detection par le prix :
   // Si le prix a atteint le niveau TP de T1 → T1 est ferme en profit
   if(g_t2_direction == 1)  // BUY
     {
      double tp1_level = g_t2_open_price + tp1_dist;
      double sl_level  = g_t2_open_price - sl_dist;

      if(curBid >= tp1_level)  // prix a atteint ou depasse le TP T1
        {
         g_t1_closed_profit = true;
         Print(StringFormat("[T1 TP TOUCHE] Prix:%.3f >= TP1:%.3f -> BE active T2 a +%d pips",
               curBid, tp1_level, InpBE_Offset));
        }
      else if(curBid <= sl_level)  // SL touche
        {
         Print(StringFormat("[T1 SL TOUCHE] Prix:%.3f <= SL:%.3f -> Fermeture T2",
               curBid, sl_level));
         FermerT2();
        }
     }
   else  // SELL
     {
      ...
      if(curAsk <= tp1_level)  // prix a atteint le TP T1
        {
         g_t1_closed_profit = true;
         Print(StringFormat("[T1 TP TOUCHE] Prix:%.3f <= TP1:%.3f -> BE active T2 a +%d pips",
               curAsk, tp1_level, InpBE_Offset));
        }
      else if(curAsk >= sl_level)  // SL touche
        {
         Print(StringFormat("[T1 SL TOUCHE] Prix:%.3f >= SL:%.3f -> Fermeture T2",
               curAsk, sl_level));
         FermerT2();
