...
//|                         Expert Advisor — Version 2.7             |
//|                                                                   |
//|  DEUX MODES (InpUseT2) :                                         |
//|  false = Position unique : SL/TP fixes classique                 |
//|  true  = Double position :                                       |
//|    T1 : SL=2500 pips | TP=4000 pips (fixe, garanti)             |
//|    T2 : SL=2500 pips | TP illimite  (BE + Trail par etapes)     |
//|    Quand T1 atteint TP : BE sur T2 a +BE_Offset pips            |
//|    Trail : +TrailStep pips a chaque step au-dela du TP T1        |
//|                                                                   |
...
//+------------------------------------------------------------------+
#property copyright "EMA_ADX_Scalper_EA v2.7"
...
input double InpLotSize      = 0.01;   // Lot par position
input int    InpSL_Pips      = 2500;   // Stop Loss en pips
input int    InpTP_Pips      = 4000;   // Take Profit en pips
input bool   InpUseT2        = true;   // true=double position T1+T2 | false=position unique
input int    InpCooldown     = 6;      // Barres minimum entre signaux
input bool   InpOneTradeOnly = true;   // Un seul trade (ou groupe) a la fois

input group "=== BE et Trailing T2 ==="
// BE sur T2 est declenche automatiquement quand T1 atteint son TP
// Le SL de T2 est pose a InpBE_Offset pips au-dessus de l entree
// Trail demarre apres le BE, avance par etapes fixes
input int    InpBE_Offset    = 3000;   // BE pose a X pips de l entree (lock profit) ** optimal
input int    InpTrailStep    = 500;    // Etape trail en pips apres le BE ** optimal
// Exemple : TP T1 atteint → SL T2 = entree + 2000 pips (+20$ garanti)
//           profit T2 = 5000 pips → SL = entree + 2000 + 1000 = 3000 pips
//           profit T2 = 6000 pips → SL = entree + 2000 + 2000 = 4000 pips
...
double   g_t2_open_price    = 0;      // prix entree T2
int      g_t2_direction     = 0;      // 1=BUY -1=SELL
ulong    g_t1_ticket        = 0;      // ticket T1 pour suivi precis
...
int OnInit()
  {
   ...
   Print("Mode : ", InpUseT2 ? "DOUBLE POSITION (T1+T2)" : "POSITION UNIQUE");
   if(InpUseT2)
     {
      Print("T1 Magic:", InpMagic, " | T2 Magic:", InpMagic+1);
      Print("SL:", InpSL_Pips, "pips | TP T1:", InpTP_Pips,
            "pips | BE_Offset:", InpBE_Offset, "pips | TrailStep:", InpTrailStep, "pips");
     }
   else
...
void OnTick()
  {
   // Gestion T2 (BE + Trail) — seulement si mode double position
   if(InpUseT2)
     {
      GererT2();
      VerifierT1Ferme();
   ...
   if(InpOneTradeOnly)
     {
      if(InpUseT2 && GroupActif()) return;
      if(!InpUseT2)
        {
         // Mode position unique — verifier Magic T1 seulement
      ...
      if(trade.Buy(InpLotSize, _Symbol, ask, sl, tp1, "T1_BUY"))
        {
         g_adxState  = 0;
         g_lastTime  = barTime;
         g_t1_ticket = trade.ResultOrder();
         Print(StringFormat("[T1 BUY] %s Prix:%.3f | SL:%.3f | TP:%.3f (+%d pips) | RSI:%.1f | ticket:#%d",
               jourNom[dt.day_of_week], ask, sl, tp1, InpTP_Pips, rsiVal, g_t1_ticket));

         // T2 — optionnel selon InpUseT2
         if(InpUseT2)
           {
            ...
            if(trade.Buy(InpLotSize, _Symbol, ask, sl, 0, "T2_BUY"))
              {
               ...
               g_t2_open_price    = ask;
               g_t2_direction     = 1;
               Print(StringFormat("[T2 BUY] %s Prix:%.3f | SL:%.3f | BE:%d pips | Trail:%d pips",
                     jourNom[dt.day_of_week], ask, sl, InpBE_Offset, InpTrailStep));
              }
           }
        }
     }
      ...
      if(trade.Sell(InpLotSize, _Symbol, bid, sl, tp1, "T1_SELL"))
        {
         ...
         g_lastTime  = barTime;
         g_t1_ticket = trade.ResultOrder();
         Print(StringFormat("[T1 SELL] %s Prix:%.3f | SL:%.3f | TP:%.3f (-%d pips) | RSI:%.1f | ticket:#%d",
               jourNom[dt.day_of_week], bid, sl, tp1, InpTP_Pips, rsiVal, g_t1_ticket));
            ...
            if(trade.Sell(InpLotSize, _Symbol, bid, sl, 0, "T2_SELL"))
              {
               ...
               g_t2_open_price    = bid;
               g_t2_direction     = -1;
               Print(StringFormat("[T2 SELL] %s Prix:%.3f | SL:%.3f | BE:%d pips | Trail:%d pips",
                     jourNom[dt.day_of_week], bid, sl, InpBE_Offset, InpTrailStep));
              }
           }
        }
     }
...
//+------------------------------------------------------------------+
//  OnTradeTransaction — appele par MT5 a chaque deal
//  Fonctionne en LIVE et en BACKTESTING (Strategy Tester)
//  C est la methode la plus fiable pour detecter la fermeture de T1
//+------------------------------------------------------------------+
void OnTradeTransaction(const MqlTradeTransaction &trans,
                        const MqlTradeRequest     &request,
                        const MqlTradeResult      &result)
  {
   // On cherche uniquement les deals de sortie (fermeture de position)
      ...
      if(profit > 0)
        {
         ...
         Print(StringFormat("[T1 TP TOUCHE] deal#%d profit=+%.2f$ -> BE active sur T2 (+%d pips)",
               deal_ticket, profit, InpBE_Offset));
        }
      else
        {
         Print(StringFormat("[T1 SL TOUCHE] deal#%d profit=%.2f$ -> Fermeture T2",
               deal_ticket, profit));
         FermerT2();
        }
     }
...
//+------------------------------------------------------------------+
//  VERIFIER SI T2 EST ENCORE ACTIF (appelée dans OnTick)
//+------------------------------------------------------------------+
void VerifierT1Ferme()
  {
   if(g_t1_open_time == 0) return;

   // Verifier si T2 existe encore
   bool t2_open = false;
   for(int p = 0; p < PositionsTotal(); p++)
     {
      ulong ticket = PositionGetTicket(p);
      if(!PositionSelectByTicket(ticket)) continue;
      if(PositionGetInteger(POSITION_MAGIC) == (InpMagic+1) &&
         PositionGetString(POSITION_SYMBOL) == _Symbol)
        { t2_open = true; break; }
     }

   // Si T2 n existe plus → reset complet
   if(!t2_open && !GroupActif())
     {
      g_t1_open_time     = 0;
      g_t1_closed_profit = false;
      g_t2_open_price    = 0;
   ...
   for(int p = PositionsTotal() - 1; p >= 0; p--)
     {
      ...
      // SL pose a InpBE_Offset pips de l entree (pas a 0)
      // profit minimum garanti sur T2 = InpBE_Offset pips
      //--------------------------------------------------------------
      if(g_t1_closed_profit)
        {
         double be_offset = InpBE_Offset * pip_size;

         if(posType == POSITION_TYPE_BUY)
           {
            double be_sl = NormalizeDouble(openPrice + be_offset, _Digits);
            if(currentSL < be_sl)
              {
               newSL = be_sl;
               Print(StringFormat("[BE T2 BUY]  #%d SL:%.3f -> %.3f (+%d pips garanti)",
                     ticket, currentSL, newSL, InpBE_Offset));
              }
           }
         else
           {
            ...
            if(currentSL > be_sl || currentSL == 0)
              {
               newSL = be_sl;
               Print(StringFormat("[BE T2 SELL] #%d SL:%.3f -> %.3f (+%d pips garanti)",
                     ticket, currentSL, newSL, InpBE_Offset));
              }
           }

         //-----------------------------------------------------------
         // TRAIL PAR ETAPES — compte depuis le TP T1 (pas depuis entree)
         //
         // Au moment ou T1 atteint son TP :
         //   SL T2 pose a : entree + BE_Offset  (minimum garanti)
         //
         // Ensuite chaque InpTrailStep pips AU-DELA du TP T1 :
         //   SL avance d un step supplementaire
         //
         // Exemple BE=2000 TrailStep=900 TP1=4000 :
         //   profit T2 = 4000 → SL = entree + 2000  ← BE fixe
         //   profit T2 = 4900 → SL = entree + 2900  ← +1 step
         //   profit T2 = 5800 → SL = entree + 3800  ← +2 steps
         //   profit T2 = 6700 → SL = entree + 4700  ← +3 steps
         //
         // BE_Offset=1000 vs BE_Offset=3000 donne des SL differents ✅
         //-----------------------------------------------------------
         double tp1_dist    = InpTP_Pips * pip_size;
         double beyond_tp1  = profit_dist - tp1_dist;  // profit apres TP T1

         int    etapes        = (beyond_tp1 > 0)
                                ? (int)MathFloor(beyond_tp1 / trail_dist)
                                : 0;
         double trail_sl_dist = be_offset + etapes * trail_dist;
