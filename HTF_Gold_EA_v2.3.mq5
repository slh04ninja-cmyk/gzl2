...
input bool              InpH1_UseCurrentBar = true;          // [EMA] Confirmer biais avec prix actuel vs Open H1[0]
...
int OnInit()
{
   ...
   PrintFormat("HTF Gold EA v2.3 [MER-L1] | %s | H1: %s | SL/TP: %s",
...
int GetH1Bias_EMA()
{
   ...
   // CONDITION 1 (obligatoire) : barre H1 fermée confirmée = index [1]
   int biasConfirmed = 0;
   if(h1_ema_fast[1] > h1_ema_slow[1]) biasConfirmed =  1; // BULLISH
   if(h1_ema_fast[1] < h1_ema_slow[1]) biasConfirmed = -1; // BEARISH
   if(biasConfirmed == 0) return 0; // EMAs croisées = pas de biais

   // CONDITION 2 (optionnelle) : prix actuel confirme direction de H1[0]
   if(InpH1_UseCurrentBar)
   {
      double currentPrice = SymbolInfoDouble(symbol, SYMBOL_BID);
      double h1Open       = iOpen(symbol, PERIOD_H1, 0);

      // Biais BULLISH → prix actuel doit être au-dessus de l'open H1[0]
      if(biasConfirmed ==  1 && currentPrice < h1Open) return 0;
      // Biais BEARISH → prix actuel doit être en-dessous de l'open H1[0]
      if(biasConfirmed == -1 && currentPrice > h1Open) return 0;
   }

   return biasConfirmed;
}
